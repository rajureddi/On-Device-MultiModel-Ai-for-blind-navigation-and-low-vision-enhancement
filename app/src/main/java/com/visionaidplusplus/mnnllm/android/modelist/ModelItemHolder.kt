...
import com.visionaidplusplus.mls.api.download.DownloadInfo
import com.visionaidplusplus.mls.api.download.DownloadState
...
import com.visionaidplusplus.mnnllm.android.utils.DialogUtils
import com.visionaidplusplus.mnnllm.android.utils.FileUtils
import com.visionaidplusplus.mnnllm.android.widgets.ModelAvatarView
import com.visionaidplusplus.mnnllm.android.widgets.TagsLayout
...
class ModelItemHolder(
    ...
    private fun getDisplayTags(modelItem: ModelItem): List<String> {
        return com.visionaidplusplus.mnnllm.android.modelmarket.TagMapper.getDisplayTagList(modelItem.tags).take(3)
    }
    ...
    private fun updateButtonAndStatus(modelWrapper: ModelItemWrapper) {
        ...
        if (modelWrapper.hasUpdate) {
            ...
            val modelId = modelWrapper.modelItem.modelId
            val isVisual = if (modelId != null) ModelListManager.isVisualModel(modelId) else false
            tvStatus.text = if (formattedSize.isNotEmpty()) {
                if (isVisual) {
                    context.getString(R.string.downloaded_click_to_capture_explain, formattedSize)
                } else {
                    context.getString(R.string.downloaded_click_to_chat, formattedSize)
                }
            } else {
                if (isVisual) {
                    context.getString(R.string.downloaded_click_to_capture_explain, "")
                } else {
    ...
    override fun onClick(v: View) {
        ...
        if (modelId != null && ModelListManager.isVisualModel(modelId)) {
            // For visual models, start capture & explain flow after download
            ChatRouter.startCaptureAndExplain(context, modelId, modelItem.localPath)
        } else {
